...
ggplot(wine_clean |> filter(country == "Italy"), aes(x = variety))+
  geom_bar()+
  facet_wrap(~r_w)
...
ggplot(wine_clean |> filter(country == "France"), aes(x = variety))+
  geom_bar()+
  facet_wrap(~r_w)
...
ggplot(wine_n, aes(x = price, y = points))+
  geom_hex()+
  scale_color_viridis_c()

  
ggplot(wine_clean |> filter(country == "New Zeaaland"), aes(x = variety))+
  geom_bar()+
  facet_wrap(~r_w)
...
nz_plot <- ggplot(data = newz_sb, aes(x = price, y = points, label = title))+
  geom_point()+
  geom_jitter(width = .05)
  theme_minimal()
